...
class T2IIterableDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        for _ in range(self.samples_div_gpus_workers_batchsize_2batches):
            ...
            if self.condition_folder:
                cond_B3HW = torch.stack([item[2] for item in batch_data])
            else:
                cond_B3HW = None
            yield (images, captions, cond_B3HW)
            del batch_data
            del images
            del captions
            del cond_B3HW
    ...
    def prepare_model_input(self, data_item) -> Tuple:
        ...
        if self.image_folder is None or not os.path.exists(self.image_folder):
            ...
            img_path, h_div_w = os.path.join(self.image_folder, data_item['image_name']), data_item['h_div_w']
        if self.condition_folder and os.path.exists(self.condition_folder):
            if abs_path:
                cond_path = os.path.join(self.condition_folder, os.path.basename(data_item['image_path']))
            else:
                cond_path = os.path.join(self.condition_folder, os.path.basename(data_item['image_name']))
        else:
            cond_path = None
            ...
            if self.load_vae_instead_of_image:
                ...
                if self.condition_folder:   
                    with open(cond_path, 'rb') as f:
                        cond: PImage.Image = PImage.open(f)
                        cond = cond.convert('RGB')
                    tgt_h, tgt_w = dynamic_resolution_h_w[h_div_w_template][self.pn]['pixel']
                    cond_B3HW = transform(cond, tgt_h, tgt_w)
                else:
                    cond_B3HW = None
        ...
        elif self.condition_folder:
            return True, (text_input, img_B3HW, cond_B3HW)
        else:
