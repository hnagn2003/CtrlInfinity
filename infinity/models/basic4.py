...
class SelfAttention(nn.Module):
    def __init__(
        ...
        self.q_bias, self.v_bias = nn.Parameter(torch.zeros(embed_dim)), nn.Parameter(torch.zeros(embed_dim)) # embed_dim=2048
    ...
    def forward(self, x, attn_bias_or_two_vector: Union[torch.Tensor, Tuple[torch.IntTensor, torch.IntTensor]], attn_fn=None, scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0, adapter=None, adapter_gate=None):
        ...
        _, wk, wv = self.mat_qkv.weight.chunk(3)
        if adapter is not None:
            adapter_k = F.linear(input=adapter, weight=wk, bias=self.zero_k_bias).view(B, L, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
            adapter_v = F.linear(input=adapter, weight=wv, bias=self.v_bias).view(B, L, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        if self.using_flash: q, k, v = qkv.unbind(dim=2); L_dim = 1           # q or k or v: all are shaped in (B:batch_size, L:seq_len=pn*pn, H:heads=16, c:head_dim=128)
        ...
        if self.using_flash:
            ...
            if self.use_flex_attn and attn_fn is not None:
                ...
                if adapter is not None:
                    adapter_out = attn_fn(q, adapter_k,  adapter_gate.tanh() * adapter_v, scale=self.scale).transpose(1, 2).reshape(B, L, C)
                    oup = oup + adapter_out
                ...
                if adapter is not None:
                    adapter_out = slow_attn(query=q, key=adapter_k, value= adapter_gate.tanh() * adapter_v, scale=self.scale, attn_mask=attn_bias_or_two_vector, dropout_p=0).transpose(1, 2).reshape(B, L, C)
                    oup = oup + adapter_out
            # oup: bf16
...
class CrossAttention(nn.Module):
    def __init__(
        ...
        if for_attn_pool:
            q_adapter = torch.empty(1, self.num_heads, self.head_dim)
            nn.init.trunc_normal_(q_adapter, mean=0, std=math.sqrt(1 / embed_dim / 3))
            self.mat_q_adapter = nn.Parameter(q_adapter)
        else:
            self.mat_q_adapter = nn.Linear(embed_dim, embed_dim, bias=True)
    ...
    def forward(self, q, ca_kv, adapter=None, adapter_gate=None):
        """
        ...
        """
        kv_text, cu_seqlens_k, max_seqlen_k = ca_kv
        N = kv_text.shape[0]
        
        kv_compact = F.linear(kv_text, weight=self.mat_kv.weight, bias=torch.cat((self.zero_k_bias, self.v_bias))).view(N, 2, self.num_heads, self.head_dim) # NC => N2Hc
        ...
        if not self.for_attn_pool:
            ...
            if adapter is not None:
                q_compact += self.mat_q_adapter(adapter).view(-1, self.num_heads, self.head_dim)
        else:
...
class CrossAttnBlock(nn.Module):
    ...
    # NOTE: attn_bias_or_two_vector is None during inference
    def forward(self, x, cond_BD, ca_kv, attn_bias_or_two_vector, attn_fn=None, scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0, adapter=None, adapter_gate=None):    # todo: minGPT and vqgan also uses pre-norm, just like this, while MaskGiT uses post-norm
        ...
        if self.fused_norm_func is None:
            ...
            x = x + self.ca(self.ca_norm(x), ca_kv, adapter=adapter, adapter_gate=adapter_gate).float().mul_(self.ca_gamma)
            x = x + self.drop_path(self.ffn( self.ln_wo_grad(x.float()).mul(scale2.add(1)).add_(shift2) ).mul(gamma2)) # this mul(gamma2) cannot be in-placed cuz we possibly use FusedMLP
        else: #go this path
            ...
            x = x + self.drop_path(x_sa.mul_(gamma1))
            x = x + self.ca(self.ca_norm(x), ca_kv, adapter, adapter_gate).float().mul_(self.ca_gamma)
