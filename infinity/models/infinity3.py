...
class MultipleLayers(nn.Module):
    ...
    def forward(self, x, cond_BD, ca_kv, attn_bias_or_two_vector, attn_fn=None, scale_schedule=None, checkpointing_full_block=False, rope2d_freqs_grid=None, adapter=None, adapter_gate=None):
        h = x

        for m in self.module:
            if checkpointing_full_block:
                h = torch.utils.checkpoint.checkpoint(m, h, cond_BD, ca_kv, attn_bias_or_two_vector, attn_fn, scale_schedule, rope2d_freqs_grid, use_reentrant=False, adapter=adapter, adapter_gate=adapter_gate)
            else:
                h = m(h, cond_BD, ca_kv, attn_bias_or_two_vector, attn_fn, scale_schedule, rope2d_freqs_grid, adapter=adapter, adapter_gate=adapter_gate)
        return h
class AdapterGate(nn.Module):
    def __init__(self, num_heads, attn_head_dim):
        super(AdapterGate, self).__init__()
        # Initialize weight and bias as parameters
        self.weight = nn.Parameter(torch.ones(1, num_heads, 1, attn_head_dim))
        self.bias = nn.Parameter(torch.zeros(1, num_heads, 1, attn_head_dim))  # typically bias is initialized to zero

    def forward(self, x):
        # Define forward pass if necessary
        return self.weight * x + self.bias
def small_init(module):
    ...
    return module
class ImageInstructionAdapter(nn.Module):
    def __init__(self, C, num_heads, depth, vae_C):
        super().__init__()
        self.C, self.num_heads, self.depth = C, num_heads, depth
        # self.adapter_gate = torch.nn.Parameter(torch.ones(1, self.num_heads, 1, 1))
        self.adapter_gate = small_init(AdapterGate(self.num_heads, self.C//self.num_heads))
        ...
        self.image_proj2 = nn.ModuleList([
            nn.Linear(vae_C, vae_C*2), 
            nn.Linear(vae_C*2, vae_C*2), 
            nn.Linear(vae_C*2, vae_C)
        ])
...
class Infinity(nn.Module):
    def __init__(
        ...
        if use_image_adapter:
            self.image_adapter = ImageInstructionAdapter(self.C, self.num_heads, self.depth, self.d_vae)
    ...
    def prepare_cond_BLC(self, cond_features, vae_scale_schedule, return_cat_x=True):
        scaled_cond_features = []
        for si in range(len(vae_scale_schedule)-1):
            this_scale_cond = F.interpolate(cond_features, size=(vae_scale_schedule[si+1][1], vae_scale_schedule[si+1][2]), mode='bilinear', align_corners=False)
            this_scale_cond = this_scale_cond.reshape(this_scale_cond.shape[0], this_scale_cond.shape[1], -1).permute(0, 2, 1)
            scaled_cond_features.append(this_scale_cond)
        if return_cat_x:
            return torch.cat(scaled_cond_features, dim=1)
        return scaled_cond_features
    ...
    def forward(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC_wo_prefix: torch.Tensor, scale_schedule: List[Tuple[int]],
        cfg_infer=False, cond_BLC_wo_prefix = None,
        **kwargs,
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            if cond_BLC_wo_prefix is not None:
                for i in range(len(self.image_adapter.image_proj2)):
                    cond_BLC_wo_prefix = self.image_adapter.image_proj2[i](cond_BLC_wo_prefix)
            ...
            # l_end = L, self.pad_to_multiplier = 128, 
            if self.customized_flash_attn:
                ...
                if need_to_pad:
                    x_BLC = F.pad(x_BLC, (0, 0, 0, need_to_pad)) # B, 640, dim
                    if instruct_BLC is not None:
                        instruct_BLC = F.pad(instruct_BLC, (0, 0, 0, need_to_pad))
        ...
        if self.num_block_chunks == 1:
            ...
            for i, chunk in enumerate(self.block_chunks): # this path
                if self.add_lvl_embeding_only_first_block and i == 0:
                    x_BLC = self.add_lvl_embeding_for_x_BLC(x_BLC, scale_schedule, need_to_pad)
                    if instruct_BLC is not None:
                        instruct_BLC = self.add_lvl_embeding_for_x_BLC(instruct_BLC, scale_schedule, need_to_pad)
                if not self.add_lvl_embeding_only_first_block:
                    x_BLC = self.add_lvl_embeding_for_x_BLC(x_BLC, scale_schedule, need_to_pad)
                    if instruct_BLC is not None:
                        instruct_BLC = self.add_lvl_embeding_for_x_BLC(instruct_BLC, scale_schedule, need_to_pad)
                if instruct_BLC is not None:
                    instruct_BLC = self.image_adapter.adapter_query[i](instruct_BLC)
                x_BLC = chunk(x=x_BLC, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=attn_bias_or_two_vector, attn_fn=attn_fn, scale_schedule=scale_schedule, checkpointing_full_block=checkpointing_full_block, rope2d_freqs_grid=self.rope2d_freqs_grid, adapter=instruct_BLC, adapter_gate=self.image_adapter.adapter_gate if hasattr(self, 'image_adapter') else None)
    ...
    def autoregressive_infer_cfg(
        ...
        if condition_path is not None:
            cond_B3HW = Image.open(condition_path).convert('RGB')
            cond_B3HW = transform(cond_B3HW, tgt_h=tgt_h, tgt_w=tgt_w).unsqueeze(0)
        else:
            cond_B3HW = None
        ...
        kv_compact, lens, cu_seqlens_k, max_seqlen_k = label_B_or_BLT
        #kv_compact: text tokenized (len_tokens,embed_dim), lens: len_tokens, cu_seqlens_k: cumulative  len_tokens, max_seqlen_k: max text length
        ...
        if any(np.array(cfg_list) != 1):
            ...
            if not negative_label_B_or_BLT:
                ...
                for le in lens:
                    ...
                    total += le
                kv_compact = torch.cat((kv_compact, kv_compact_un), dim=0) # [len_tokens*2, 1024]
                cu_seqlens_k = torch.cat((cu_seqlens_k, cu_seqlens_k[1:]+cu_seqlens_k[-1]), dim=0) #(0, len_tokens, len_tokens*2)
            else:
        ...
        sos = cond_BD = self.text_proj_for_sos((kv_compact, cu_seqlens_k, max_seqlen_k)) # sos shape: [2B, 2048]
        kv_compact = self.text_proj_for_ca(kv_compact) # kv_compact shape: [2*len_tokens, 4096]
        ca_kv = kv_compact, cu_seqlens_k, max_seqlen_k
        last_stage = sos.unsqueeze(1).expand(bs, 1, -1) + self.pos_start.expand(bs, 1, -1)
        cond_first_stage = sos.unsqueeze(1).expand(bs, 1, -1) + self.pos_start.expand(bs, 1, -1)

        with torch.amp.autocast('cuda', enabled=False):
            cond_BD_or_gss = self.shared_ada_lin(cond_BD.float()).float().contiguous() #B,1,6,2048
        ...
        if cond_B3HW is not None:
            ...
            cond_BLC_wo_prefix = self.prepare_cond_BLC(cond_raw_features, vae_scale_schedule, return_cat_x=False)
            for j in range(len(cond_BLC_wo_prefix)):
                for i in range(len(self.image_adapter.image_proj2)):
                    cond_BLC_wo_prefix[j] = self.image_adapter.image_proj2[i](cond_BLC_wo_prefix[j])
            ...
            cond_BLC_wo_prefix = [cond_first_stage] + cond_BLC_wo_prefix
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            ...
            need_to_pad = 0
            attn_fn = None
            if self.use_flex_attn and not eval_mode:
                # if need_to_pad:
            ...
            layer_idx = 0
            for block_idx, b in enumerate(self.block_chunks): # len(self.block_chunks)=8, self.block_chunks[0] = 4xCrossAttentionBlock (se,ca,ffn)
                # last_stage shape: [4, pn*2, 2048], cond_BD_or_gss.shape: [4, pn*2, 6, 2048], ca_kv[0].shape: [64, 2048], ca_kv[1].shape [5], ca_kv[2]: int
                if not self.add_lvl_embeding_only_first_block or block_idx == 0:
                    last_stage = self.add_lvl_embeding(last_stage, si, scale_schedule, need_to_pad=need_to_pad)
                    if cond_B3HW is not None:
                        cond_BLC = cond_BLC_wo_prefix[si] # [B,?,2048]
                        # if cond_need_to_pad:
                        #     cond_BLC = F.pad(cond_BLC, (0, 0, 0, cond_need_to_pad))
                        cond_BLC = self.add_lvl_embeding(cond_BLC, si, scale_schedule, need_to_pad=0)
                    else:
                        cond_BLC = None

                if isinstance(b.module, MultipleLayers):
                    b_module = b.module.module
                else:
                    b_module = b.module
                if cond_BLC is not None:
                    cond_BLC = self.image_adapter.image_proj(cond_BLC)
                    cond_BLC = self.image_adapter.image_norm(cond_BLC)
                for m in b_module:
                    if cond_BLC is not None:
                        cond_BLC = self.image_adapter.adapter_query[block_idx](cond_BLC)
                    last_stage = m(x=last_stage, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=None, attn_fn=attn_fn, scale_schedule=scale_schedule, rope2d_freqs_grid=self.rope2d_freqs_grid, scale_ind=si, adapter=cond_BLC, adapter_gate=self.image_adapter.adapter_gate if cond_BLC is not None else None)
            ...
            if (cfg != 1) and add_cfg_on_logits:
                ...
                logits_BlV = self.get_logits(last_stage, cond_BD).mul(1/tau_list[si]) #(2B, pn*pn, vocab_size), temperature <1 for more determistic, less diverge 
                logits_BlV = cfg * logits_BlV[:B] + (1-cfg) * logits_BlV[B:] # (B, pn*pn, vocab_size)
            else:
                logits_BlV = self.get_logits(last_stage[:B], cond_BD[:B]).mul(1/tau_list[si])
            
            if self.use_bit_label:
                tmp_bs, tmp_seq_len = logits_BlV.shape[:2]
                logits_BlV = logits_BlV.reshape(tmp_bs, -1, 2) #B, codebook_dim*pn*pn, 2
                idx_Bld = sample_with_top_k_top_p_also_inplace_modifying_logits_(logits_BlV, rng=rng, top_k=top_k or self.top_k, top_p=top_p or self.top_p, num_samples=1)[:, :, 0] #B, codebook_dim*pn*pn
                idx_Bld = idx_Bld.reshape(tmp_bs, tmp_seq_len, -1) # B, pn*pn, codebook_dim
            else:
            ...
            if vae_type != 0:
                ...
                if si != num_stages_minus_1: #d: VAE latent dimension
                    # f_hat (B, d, 1, h, w)
