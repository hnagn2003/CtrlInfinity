...
class Infinity(nn.Module):
    def __init__(
        ...
        raw_scale_schedule=(1, 2, 3, 4, 5, 6, 8, 10, 13, 16), #None ...
        head_depth=1,
        top_p=0.0, top_k=0.0,
        ...
        video_frames=1,
        always_training_scales=20, #100 2
        apply_spatial_patchify = 0,
        inference_mode=False, #1
    ...
    def autoregressive_infer_cfg(
        ...
        if any(np.array(cfg_list) != 1):
            ...
            if not negative_label_B_or_BLT:
                ...
                total = 0
                for le in lens:
                    if self.cfg_uncond.dim() < 2:
                        cfg_uncond = self.cfg_uncond.view(-1, self.C)
                    else:
                        cfg_uncond = self.cfg_uncond
                    kv_compact_un[total:total+le] = cfg_uncond[:le]
                    total += le
        ...
        # if kv_compact.dim() > 1:
        #     kv_compact = kv_compact.view(kv_compact.shape[0] * kv_compact.shape[1])
        try:
            ...
            print(self.text_proj_for_sos.ca.mat_kv.weight.shape)
            print(self.text_proj_for_sos.ca.mat_q.shape)
            breakpoint()
            raise e
        ca_kv = kv_compact, cu_seqlens_k, max_seqlen_k
