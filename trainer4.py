...
class InfinityTrainer(object):
    ...
    def train_step(
        ...
        inp_B3HW: FTen, text_cond_tuple: Union[ITen, FTen], args: arg_util.Args, cond_B3HW: FTen=None
        ...
        with self.gpt_opt.amp_ctx:
            with torch.amp.autocast('cuda', enabled=False):
                with torch.no_grad():
                    ...
                    raw_features, _, _ = self.vae_local.encode_for_raw_features(inp_B3HW, scale_schedule=vae_scale_schedule) #B, vae_c, ph, pw
                    if cond_B3HW is not None:
                        cond_raw_features, _, _ = self.vae_local.encode_for_raw_features(cond_B3HW, scale_schedule=vae_scale_schedule)
            ...
            if cond_B3HW is not None:
                cond_BLC_wo_prefix, _ = self.bitwise_self_correction.flip_requant(vae_scale_schedule, cond_B3HW, cond_raw_features, device)
            # truncate scales
            training_scales = args.always_training_scales
            training_seq_len = np.array(scale_schedule)[:training_scales].prod(axis=1).sum()
            x_BLC_wo_prefix = x_BLC_wo_prefix[:, :(training_seq_len-np.array(scale_schedule[0]).prod()), :] #B, L, log2(vocab_size)
            if cond_B3HW is not None:
                cond_BLC_wo_prefix = cond_BLC_wo_prefix[:, :(training_seq_len-np.array(scale_schedule[0]).prod()), :]
            else:
                cond_BLC_wo_prefix = None

            logits_BLV = self.gpt(text_cond_tuple, x_BLC_wo_prefix, scale_schedule=scale_schedule[:training_scales], cond_BLC_wo_prefix=cond_BLC_wo_prefix) # [bs, 1*1+...+64*64, vocab_size or log2(vocab_size)*2]
            ...
            if args.use_bit_label:
                ...
                loss = self.train_loss(logits_BLV.reshape(tmp_bs, tmp_seq_len, -1, 2).permute(0,3,1,2), gt_BL) #logit (B,2,seq_len,codebook_dim)
        ...
        # print(" >>>>>>>>>>>>>>>> trainable parameters <<<<<<<<<<<<<<<<<<")
        # for name, param in self.gpt_wo_ddp.named_parameters():
